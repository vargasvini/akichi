...
class StreamInput(val context: Context, val preferences: Preferences)
{
	...
	// Sony DualSense (vendor 0x054C / product 0x0CE6). On Fire OS / Android 11-12 the
	// system keylayout delivers a scrambled button/axis mapping for this controller
	// (see CONTROLLER_DIAGNOSIS.md). When enabled, we undo that scramble.
	private var dualSenseDeviceId = Int.MIN_VALUE
	private var dualSenseDeviceMatch = false
	private fun isFireTvDualSense(device: InputDevice?): Boolean
	{
		if(!controllerMapFix || device == null)
			return false
		if(device.id != dualSenseDeviceId)
		{
			dualSenseDeviceId = device.id
			dualSenseDeviceMatch = device.vendorId == 0x054C && device.productId == 0x0CE6
		}
		return dualSenseDeviceMatch
	}
	...
	fun dispatchKeyEvent(event: KeyEvent): Boolean
	{
		...
		if(isFireTvDualSense(event.device))
			return dispatchKeyEventFireTvDualSense(event)
	...
	fun onGenericMotionEvent(event: MotionEvent): Boolean
	{
		if (!event.isFromSource(InputDevice.SOURCE_CLASS_JOYSTICK) && (event.action == MotionEvent.ACTION_BUTTON_PRESS || event.action == MotionEvent.ACTION_BUTTON_RELEASE || event.action == MotionEvent.ACTION_MOVE || event.action == MotionEvent.ACTION_UP || event.action == MotionEvent.ACTION_DOWN)) {
			...
			// On the broken Fire OS DualSense layout the analog triggers are reported on
			// AXIS_RX/AXIS_RY instead of AXIS_LTRIGGER/AXIS_RTRIGGER (sticks stay on X/Y/Z/RZ).
			val fix = isFireTvDualSense(event.device)
			val l2Axis = if(fix) MotionEvent.AXIS_RX else MotionEvent.AXIS_LTRIGGER
			val r2Axis = if(fix) MotionEvent.AXIS_RY else MotionEvent.AXIS_RTRIGGER
			...
			motionControllerState.l2State = event.getAxisValue(l2Axis).unsignedAxis()
			motionControllerState.r2State = event.getAxisValue(r2Axis).unsignedAxis()
